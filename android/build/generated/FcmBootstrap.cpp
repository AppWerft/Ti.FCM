...
static void Fcm_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Fcm.getBinding requires 1 argument: binding");
...
static void Fcm_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Fcm_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Fcm_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
...
static titanium::bindings::BindEntry FcmBinding = {
	"ti.fcm",
	Fcm_init,
	Fcm_dispose
};
...
Java_ti_fcm_FcmBootstrap_nativeBootstrap
