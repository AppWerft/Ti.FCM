...
static void TiGoosh_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "TiGoosh.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::TiGooshBindings::lookupGeneratedInit(
...
static void TiGoosh_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, TiGoosh_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void TiGoosh_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TiGooshBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TiGooshBinding = {
	"ti.goosh",
	TiGoosh_init,
	TiGoosh_dispose
};
...
Java_ti_goosh_TiGooshBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.goosh", &TiGooshBinding);
	titanium::KrollBindings::addExternalLookup(&(::TiGooshBindings::lookupGeneratedInit));
}
